...
export class Chunk {
  ...
  generateMesh() {
    ...
    this.uvs = new Float32Array(
      (this.amountVerts / 3) * 2 * this.rescaleAmount
    );
    this.uvsSize = 0;
  ...
  potentialRescaleGeometryBuffer() {
    if (this.positionsSize >= this.amountVerts * this.rescaleAmount - 2) {
      console.log(
        "Error tried to go over max verts size" +
          this.amountVerts * this.rescaleAmount
      );
      let tempPos = this.positions;
      let tempUv = this.uvs;
      this.positions = new Int16Array(
        this.amountVerts * (this.rescaleAmount + 1)
      );
      this.uvs = new Float32Array(
        (this.amountVerts / 3) * 2 * (this.rescaleAmount + 1)
      );
